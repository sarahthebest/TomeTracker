...
const BookSlider = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (bottomSliderAnimation)
                cancelAnimationFrame(bottomSliderAnimation);
        };
    }, [books]);
    ...
    return (
        <div className="relative w-full flex flex-col gap-2">
            <div className="relative w-full h-40">
                <div
                    ref={sliderRefBottom}
                    className="flex gap-10 w-max justify-center place-content-center"
                ...
                >
                    {[...books, ...books, ...books].map((book, index) => (
                        <img
                            key={index}
                            src={
                                book.imageLinks?.thumbnail || "/placeholder.jpg"
                            }
                            alt={book.title || "Book Cover"}
                            className="h-40 w-auto object-cover rounded shadow-md"
                        />
                    ))}
                </div>
            </div>
